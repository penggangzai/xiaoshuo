...
var LoginV1 = {
    ...
    init: function(params) {
        ...
        // 小阅的更多方式
        $('#j_readx_moreLogin').on('click', function() {
            $(this).hide();
            $(this).next('span').show();
        });
        
        $('#imgQrCode').on('click', function() {
            $('.error-tip').addClass('hidden');
            LoginV1.getQrCode();
        });
        
        // 切换扫码和普通登录方式
        $('#j_scanLoginBtn').on('click', function() {
        ...
        // 小阅和阅文账号切换
        $('#j_loginTab li').on('click', function() {
    ...
    login: function() {
        ...
        var data = LoginV1.buildBaseData();
            data.username = username + LoginV1.config.loginPostfix,
    ...
    qqLoginQidian: function() {
        if (LoginV1.config.code != 0) {
            var url = LoginV1.config.staticBaseUrl;
            if (LoginV1.config.type == 'wap') {
                url += '/mobile/login.html?';
            }
            url += LoginV1.buildBaseDataString();
            
            $('#error').html(LoginV1.config.message || '登陆失败，请选用其他方式登陆');
            return;
        }
