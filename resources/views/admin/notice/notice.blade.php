...
@section('link')
    <div class="container-fulid col-md-10 add">
        <h3><span class="glyphicon glyphicon-list"></span>公告管理</h3>
        <table class="table table-bordered table-hover table-striped">
            ...
            @foreach($list as $v)
                <tr class="danger">
                    <td>{{$v->id}}</td>
                    <td>{{$v->title}}</td>
                    {{--<td><a href="{{url('admin/notdisplay')}}/{{$v->id}}/{{$v->display}}" class="btn-primary btn-sm" style="color: #FFF0F0">--}}
                    {{--{{($v->display)==1?'显示':'隐藏'}}--}}
                    {{--</a></td>--}}
                    <td><a href="{{url('admin/notface')}}/{{$v->id}}/{{$v->face}}" class="btn-primary btn-sm"
                           style="color: #FFF0F0">
                            {{($v->face)==1?'上线':'下线'}}
                        </a></td>
                    <td>{{$v->time}}</td>
                        ...
                        <table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
                            ...
                            <tr>
                                <td align="left" valign="top">
                                    <form method="post" action="{{url('admin/notadd')}}" enctype='multipart/form-data'>
                                        {{  csrf_field() }}
                                        <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
                                            <tr onMouseOut="this.style.backgroundColor='#ffffff'"
                                                onMouseOver="this.style.backgroundColor='#edf5ff'">
                                                <td align="right" valign="middle"
                                                    class="borderright borderbottom bggray">公告内容:
                                                </td>
                                                <td align="left" valign="middle"
                                                    class="borderright borderbottom main-for">
                                <textarea name="title" cols="50" rows="3" style='vertical-align:top'
